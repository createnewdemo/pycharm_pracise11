...
class weater_demo(object):
    ...
    def get_url(self):#获取地区接口
        ...
        for self.inx, val in enumerate(self.infos):
            #print(inx)
            self.url = 'https://free-api.heweather.net/s6/weather/forecast?location={}&key=3f8659a2ab674ca6bdc3e740462a6ae1'.format(self.infos[self.inx]['地区'])
    ...
    def get_weather_data(self):
        ...
        for data in result:#三天的
            ...
            datas.append(Data)
        print(datas[0])
        self.text = "%s日份天气预报到账啦\n今天%s的天气情况为%s\n最高温度%s℃，最低温度%s℃\n太阳公公出来的时间大约是%s,太阳公公落下的时间大约是%s请注意合理安排时间进行学习,娱乐,锻炼。\n今天的天气预报到此,祝您有个愉快的一天" % (self.today_time,Data['city'],Data['cond'],Data['max'],Data['min'],Data['sr'],Data['ss'])
        print(self.text)
        #print(self.urls)
        pass
